content = '''export function isCipherQuestPath() {
...
function renderMidnightPortal() {
  writeRawPage("<!DOCTYPE html><html><head><title>Midnight Portal</title></head><body style=\\"background:#050505; color:#00ff00; font-family:monospace; padding:50px;\\"><script>if (!document.cookie.includes('session_token=')) { document.cookie = 'session_token=eyJyb2xlIjoiZ3Vlc3QifQ==; path=/'; } window.onload = function() { let msg = '<h2>ACCESS DENIED</h2><p>Parsing session_token... Role verified as <b>GUEST</b>.</p><p>Admin privileges required.</p>'; if (document.cookie.includes('eyJyb2xlIjoiYWRtaW4ifQ==')) { msg = '<h2>AUTH ACCEPTED</h2><p>Welcome, Administrator.</p><a href=\\"/midnightvault\\" style=\\"color:#000; background:#00ff00; padding:10px; text-decoration:none;\\">ENTER VAULT</a>'; } document.getElementById('content').innerHTML = msg; }</script><div id=\\"content\\">Verifying...</div></body></html>");
}

function renderMidnightVault() {
  writeRawPage("<!DOCTYPE html><html><head><title>Midnight Vault</title></head><body style=\\"background:#1a0000; color:#ff4444; font-family:monospace; padding:50px;\\"><script>window.bypassVault = function(token) { if(token === btoa(navigator.userAgent)) { document.getElementById('vault-door').innerHTML = 'BIOMETRIC ACCEPTED. <a href=\\"/thesubnode\\" style=\\"color:#fff;\\">PROCEED TO SUBNODE</a>'; return 'Success'; } else { console.error('Biometric mismatch. Token invalid.'); return 'Failed'; } }</script><h1 style=\\"text-align:center;\\">VAULT LOCKED</h1><div id=\\"vault-door\\" style=\\"text-align:center;\\"><p>Biometric override required.</p><p>Invoke global function <b>bypassVault(token)</b> via console.</p><p><i>System Note: The token is the Base64 encoded string of your browser\\'s exact User-Agent.</i></p></div></body></html>");
}

function renderTheSubnode() {
  writeRawPage("<!DOCTYPE html><html><head><title>The Subnode</title><meta name=\\"pin-md5\\" content=\\"02860d5b7fc4ba2f4dd17c0921dc8bfa\\"></head><body style=\\"background:#111; color:#eee; font-family:monospace; padding:40px;\\"><h2>DATABASE TERMINAL</h2><p>Enter 5-digit PIN to decrypt sector location:</p><input type=\\"text\\" id=\\"pin\\" /><button onclick=\\"check()\\">SUBMIT</button><div id=\\"hidden-route\\" style=\\"color:transparent; selection-background:transparent; margin-top:40px;\\">/thearchitectsblueprint</div><script>function check() { if(document.getElementById('pin').value === '84921') { document.getElementById('hidden-route').style.color = '#111'; alert('Decrypted successfully. Highlight the space below this box to reveal the route.'); } else { alert('INVALID PIN'); } }</script></body></html>");
}

function renderTheArchitectsBlueprint() {
  writeRawPage("<!DOCTYPE html><html><head><title>The Blueprint</title></head><body style=\\"background:#000; color:#0f0; font-family:monospace; padding:50px;\\"><h2>THE ARCHITECT\\'S TERMINAL</h2><p>The final asset is locked in secure memory.</p><p>To extract it, you must invoke the global function <b>extractFlag(param1, param2)</b> via your terminal console.</p><ul><li><b>Param 1:</b> A 5-letter string. The clearance code associated with the end of all things.</li><li><b>Param 2:</b> An integer. The exact POST number from J.T. on the Ghost_Net Forum.</li></ul><script>window.extractFlag = function(p1, p2) { if (p1 === 'OMEGA' && p2 === 3239) { return 'FLAG: thegrandarchitect'; } else { return 'CORRUPTION DETECTED. INVALID PARAMETERS.'; } }</script></body></html>");
}
