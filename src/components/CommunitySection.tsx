...
export const CommunitySection: React.FC = () => {
  ...
  return (
    <section id="community" className="relative py-28 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto border-t border-zinc-900">
      ...
      <div className="p-8 sm:p-12 rounded-3xl bg-gradient-to-br from-[#0a0c1e] via-[#070814] to-[#04040a] border border-[#5865F2]/40 hover:border-[#5865F2]/70 transition-all duration-300 relative overflow-hidden mb-8 shadow-2xl group">
        
        {/* Ambient Blur */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#5865F2]/15 rounded-full blur-3xl pointer-events-none group-hover:bg-[#5865F2]/25 transition-all"></div>

        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 relative z-10">
          
          <div className="space-y-4 max-w-xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#5865F2]/15 border border-[#5865F2]/40 text-xs font-mono text-indigo-300">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2 text-xs font-mono text-zinc-300">
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-950/80 border border-zinc-800">
                <Users className="w-3.5 h-3.5 text-indigo-400" />
                <span>Open Delegation</span>
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-950/80 border border-zinc-800">
          ...
          <div className="w-full md:w-auto flex flex-col sm:flex-row md:flex-col gap-3">
            <a
              ...
              className="px-8 py-4 rounded-full bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold text-sm transition-all shadow-[0_0_25px_rgba(88,101,242,0.5)] flex items-center justify-center gap-2 group cursor-pointer"
            >
      ...
      <div className="p-8 rounded-3xl bg-[#0a0a0f] border border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div>
          <h4 className="text-lg font-bold text-white">Subscribe to Intelligence Dispatch</h4>
          <p className="text-xs text-zinc-400 font-light mt-1">Receive challenge directives and schedule bulletins directly via email.</p>
        </div>

        {isDone ? (
          <span className="text-xs font-mono text-emerald-400 font-medium">Accreditation email registered successfully.</span>
        ) : (
          <form onSubmit={handleSubmit} className="flex items-center gap-2 w-full sm:w-auto">
            <input
              ...
              className="px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-full text-xs font-mono text-white placeholder-zinc-500 focus:outline-none focus:border-violet-500 w-full sm:w-64"
            />
            <button
              type="submit"
              onMouseEnter={() => soundFX.playHover()}
