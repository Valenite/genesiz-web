...
import { X, Shield, Copy, Check, Users, Database, FileSpreadsheet, Lock, KeyRound, AlertCircle } from 'lucide-react';
...
export const AdminVaultModal: React.FC<AdminVaultModalProps> = ({ onClose }) => {
  ...
  const handleAuthenticate = (e: React.FormEvent) => {
    ...
    // Administrative Password check: 'Valenite' (case-insensitive)
    if (password.trim().toLowerCase() === 'valenite') {
      soundFX.playSuccess();
      setIsAuthenticated(true);
    } else {
      soundFX.playClick();
      setAuthError('Access Denied: Invalid Administrative Password.');
      setPassword('');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-black/90 backdrop-blur-md animate-fadeIn font-sans">
      ...
      <div className="relative w-full max-w-4xl bg-[#08080c] border border-indigo-900/50 rounded-3xl shadow-2xl overflow-hidden z-10 max-h-[92vh] flex flex-col">
        ...
        {/* PASSWORD PROTECTION CHALLENGE SCREEN */}
        {!isAuthenticated ? (
          <form onSubmit={handleAuthenticate} className="p-8 sm:p-12 space-y-6 text-center max-w-md mx-auto my-auto">
            <div className="w-14 h-14 rounded-2xl bg-indigo-950/80 border border-indigo-500/40 flex items-center justify-center text-indigo-400 mx-auto shadow-lg">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs font-mono text-zinc-400">
                Restricted Telemetry Vault // Executive Organizers Only
              </p>
            </div>

            {authError && (
              <div className="p-3 rounded-xl bg-rose-950/80 border border-rose-800 text-rose-200 text-xs font-mono flex items-center justify-center gap-2">
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 font-mono transition-colors"
                />
              </div>

              <button
                type="submit"
                className="w-full py-3.5 rounded-xl bg-white hover:bg-zinc-200 text-black font-bold text-xs uppercase font-mono tracking-wider transition-all cursor-pointer shadow-md active:scale-95"
              >
                Unlock Registration Vault
              </button>
            </div>
          </form>
        ) : (
